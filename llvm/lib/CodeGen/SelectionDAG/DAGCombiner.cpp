...
/// Ref: D. Lemire, O. Kaser, and N. Kurz, "Faster Remainder by Direct Computation" (LKK)
SDValue DAGCombiner::foldUREM(SDNode *node) {
    SDLoc DL(node);
    EVT VT = node->getValueType(0);
    EVT FVT;
    if (VT.isVector()) {
        EVT SVT = EVT::getIntegerVT(*DAG.getContext(), VT.getScalarSizeInBits() * 2);
    ...
    if (!TLI.isOperationLegalOrCustomOrPromote(ISD::MUL, FVT) && TLI.isOperationExpand(ISD::MUL, FVT))
      return SDValue();
    ...
    SDValue MagicFactor = VT.isVector() ? DAG.getBuildVector(FVT, DL, MagicFactors) : MagicFactors[0];
    
    // lowbits = c * n
    SDValue Lowbits = DAG.getNode(ISD::MUL, DL, FVT, MagicFactor, ExtendedNumerator);
    ...
    if (LegalOperations ? TLI.isOperationLegal(ISD::MULHU, FVT) : TLI.isOperationLegalOrCustom(ISD::MULHU, FVT))
        ...
        SDValue LoHi = DAG.getNode(ISD::UMUL_LOHI, DL, DAG.getVTList(FVT, FVT), Lowbits, ExtendedDivisor);
...
SDValue DAGCombiner::foldSREM(SDNode *node) {
    ...
    EVT FVT;
    if (VT.isVector()) {
        EVT TmpVT = EVT::getIntegerVT(*DAG.getContext(), VT.getScalarSizeInBits() * 2);
        FVT = EVT::getVectorVT(*DAG.getContext(), TmpVT, VT.getVectorElementCount());
    } else {
        FVT = EVT::getIntegerVT(*DAG.getContext(), VT.getScalarSizeInBits() * 2);
    }
    ...
    if (!TLI.isOperationLegalOrCustomOrPromote(ISD::SRA, FVT) && TLI.isOperationExpand(ISD::SRA, FVT))
      return SDValue();
    ...
    auto BuildSREMPattern = [&](ConstantSDNode *DivisorConstant) {
        ...
        APInt C = APInt::getMaxValue(F).udiv(pd.zext(F)).uadd_sat(APInt(F,1)).uadd_sat(IsPow2);
    ...
    // absolute divisor
    SDValue AbsoluteDivisor = VT.isVector() ? DAG.getBuildVector(VT, DL, AbsoluteDivisors) : AbsoluteDivisors[0];
    SDValue ExtendedAbsoluteDivisor = DAG.getZExtOrTrunc(AbsoluteDivisor, DL, FVT);
    ...
    SDValue One = DAG.getConstant(1, DL, VT);
    SDValue DecrementedAbsoluteDivisor = DAG.getNode(ISD::SUB, DL, VT, AbsoluteDivisor, One);
